...
def getVariables(data):
   ...
   # twitterId = data.split(',"id":')[1].split(',"id_str')[0]
    ...
    graph.create(person)
    tweet = Node("Tweet",name="T" , created_at= created_at,id=twitterId,tweet=tweet,reply_user=in_reply_to_screen_name, reply_tweet_id = in_reply_to_user_id_str, sentiment= sentiment,subjective=subjective)
    graph.create(tweet)
    ...
    if (in_reply_to_status_id_str!=None):
        ...
        var_id = in_reply_to_status_id_str

        # for line in file:
        #  var = line

        reply_id = var_id
        reply_id_string = '"' + reply_id + '" '
        reply = Node("Reply_Tweet", name="RT", id=reply_id, screen_name=in_reply_to_screen_name)
        # graph.create(topic_tweet)
        graph.merge(reply)
        reply = graph.evaluate('match(x:Reply_Tweet{id:' + reply_id_string + '}) return x')
        #graph.create(reply)
